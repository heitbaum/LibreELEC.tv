...
- **xmlstarlet** — **a maintained fork exists after all**, and the "upstream is dead, no source fork" reading recorded here was wrong. github.com/xmlstarlet/xmlstarlet took over the frozen SourceForge tree and shipped **1.7.0 on 2024-09-07**, its first release since 2014. **LE master is already on it**: `upstream/master` carries 1.7.0 and `packages/textproc/xmlstarlet/` has no `patches/` directory at all. 1.7.0 subsumes all three patches we carried — the 2.12 compat fix (`0246`), the 2.14 `ATTRIBUTE_UNUSED` definition (`0900`) and Matthieu Crapet's 2014 `usage2c.awk` basename regexp, verified at the tag as `sub(/^.*\//, "", command_name)`, i.e. the sourceforge submission finally landed in the fork. It also **removes the libxml2 3.0 trigger**: the release isolates the deprecated node-deregister calls and declares itself prepared for 3.0, so 1.6.1's coming hard break no longer forces the decision. Option (b), standing up a `LibreELEC/xmlstarlet` fork, is **dead — do not do it**; the fork already exists. Option (c), replace with `xmllint`, stays **blocked** for the original reason, re-checked: `config/functions:1566` uses `xml ed --inplace` and `:1571` uses `xml esc`, not just xpath. **The only local delta left on dev** is an uncommitted two-line `--disable-install-docs` for host and target — 1.7.0 ships its documentation prebuilt, so it installs with no `xsltproc`/`fop` chain needed and lands docs we do not want on an appliance. Everything else in the working-tree diff (the version, the URL move to the GitHub releases, dropping the `LIBXML_CONFIG`/`--with-libxml-*-prefix` blocks now that 1.7.0 detects via pkg-config) came from upstream. That two-line change is the piece worth submitting; it also explains the revert of the earlier local attempt `2906ac976b` (`8b411a30da`) — that one tracked a git master hash and was superseded by upstream's release bump. Untested in this checkout: no 1.7.0 tarball in `sources/`. Our libxml2 is 2.15.4; 1.7.0's CI range is 2.6.27–2.15.3, one micro short.
