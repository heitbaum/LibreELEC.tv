...
- **xmlstarlet** — **a maintained fork exists after all**, and the "upstream is dead, no source fork" reading recorded here was wrong. github.com/xmlstarlet/xmlstarlet took over the frozen SourceForge tree and shipped **1.7.0 on 2024-09-07**, its first release since 2014. That subsumes every patch we carried: the 2.12 compat fix (`0246`), the 2.14 `ATTRIBUTE_UNUSED` definition (`0900`) and Matthieu Crapet's 2014 `usage2c.awk` basename regexp — verified present at the `1.7.0` tag as `sub(/^.*\//, "", command_name)`, i.e. the sourceforge submission finally landed downstream of us. It also **removes the libxml2 3.0 trigger**: the release isolates the deprecated node-deregister calls and declares itself prepared for 3.0, so the "1.6.1 will hard-break at libxml2 3.0" deadline no longer drives anything. Option (b), standing up a `LibreELEC/xmlstarlet` fork, is therefore **dead — do not do it**; the fork we would have created already exists. Option (c), replace with `xmllint`, is still **blocked** for the original reason: `config/functions:1566` uses `xml ed --inplace` and `:1571` uses `xml esc`, not just xpath. **State:** dev has an uncommitted bump to 1.7.0 that drops all three patches and replaces the `LIBXML_CONFIG`/`--with-libxml-*-prefix` blocks with `--disable-install-docs` (1.7.0 gained pkg-config detection and ships prebuilt docs); master is still on 1.6.1 with the patches. **Untested** — no 1.7.0 tarball in `sources/`, and an earlier attempt at the same move (`2906ac976b`, against a git master hash rather than a release) was reverted by `8b411a30da` with no reason recorded, so find out what broke before trusting this retry. Our libxml2 is 2.15.4; 1.7.0's CI range is 2.6.27–2.15.3, one micro short.
