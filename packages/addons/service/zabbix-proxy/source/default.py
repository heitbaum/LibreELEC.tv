...
# addon
__addon__ = xbmcaddon.Addon(id='service.zabbix-proxy')
...
if not xbmcvfs.exists(xbmcvfs.translatePath(__addonhome__ + 'etc/zabbix_proxy.conf.d/')):
    xbmcvfs.mkdirs(xbmcvfs.translatePath(__addonhome__ + 'etc/zabbix_proxy.conf.d/'))


def writeconfig():
    system("systemctl stop service.zabbix-proxy.service")
    ...
    dbname = __addon__.getSetting("DBNAME")
    ...
    file.close()
    system("systemctl start service.zabbix-proxy.service")
