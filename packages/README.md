...
| PKG_NO_REFRESH_PATCHES | - | no  | If set, `tools/refresh-patches` exits with an error pointing to the specified tool. Use for packages whose patches are generated by a dedicated script (e.g. `tools/ffmpeg/gen-patches.sh`) and must not be maintained with quilt. |
