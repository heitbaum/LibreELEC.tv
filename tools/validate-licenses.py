...
SKIP_SPDX_HEADERS = {
    ...
    "argtable3",    # tarball bundles FreeBSD getopt (BSD-2-Clause-NetBSD SPDX tag); the
                    # project's own LICENSE is BSD-3-Clause (with non-endorsement clause)
    "aml",          # GitHub archive includes wayland-protocols XML files (BSD-3-Clause
                    # SPDX tags); project COPYING is ISC
    "neatvnc",      # same: bundled wayland-protocol XML files carry BSD-3-Clause headers;
                    # project COPYING is ISC
    "wayvnc",       # same: bundled wayland-protocol XML files carry BSD-3-Clause headers;
                    # project COPYING is ISC
    "nvtop",        # bundled ini.h (BSD-3-Clause, inih by Ben Hoyt) and amdgpu_ids.h (MIT,
                    # from libdrm/AMD) appear in first-60-files scan; project is GPL-3.0-or-later
    "buildx",       # vendored Go modules include MPL-2.0 licensed files; project LICENSE
                    # is Apache-2.0
}
...
LICENCE_CONFIRMED = {
    ...
    # COPYING ships the plain GPL-2.0 deed without the "or any later version" clause.
    # Per-file SPDX headers in source files say GPL-2.0-or-later, but the operative
    # grant is the COPYING text — GPL-2.0-only is correct.
    "flashrom": "GPL-2.0-only",
...
def main():
    ...
    def _line(label, cnt):
        dots = "." * max(1, FILL - len(label))
        return f"{label}{dots} {cnt:>4}"

    print(f"\n{'='*60}", file=sys.stderr)
    print(_line("- Total rows", len(rows)), file=sys.stderr)
    print(file=sys.stderr)
    print("- SPDX Match Confidence", file=sys.stderr)
    for pct in sorted(pct_counts.keys(), reverse=True):
        print(_line(f"  - {pct:3d}%", pct_counts[pct]), file=sys.stderr)
    print(file=sys.stderr)
    print(_line("- incorrect",        incorrect), file=sys.stderr)
    print(_line("- no-source",        no_src),    file=sys.stderr)
    print(_line("- missing-tarball",  no_tb),     file=sys.stderr)
    print(_line("- tarball-error",    errors),    file=sys.stderr)
    if other:
    ...
    if args.verbose:
        def _row_sort_key(r):
            ...
            m = re.search(r'(\d+)%', s)
            if m:
                return (0, -int(m.group(1)), r["pkg_name"].lower())
            if s.startswith("incorrect"):       return (1, 0, r["pkg_name"].lower())
            if s.startswith("no-source"):       return (2, 0, r["pkg_name"].lower())
            if s == "missing-tarball":          return (3, 0, r["pkg_name"].lower())
            if s.startswith("tarball-error"):   return (4, 0, r["pkg_name"].lower())
            return (5, 0, r["pkg_name"].lower())

        def _label(status):
            m = re.search(r'(\d+)%', status)
            if m:                               return f"{int(m.group(1)):3d}%"
            if status.startswith("incorrect"):  return "FAIL"
            if status.startswith("no-source"):  return " n/a"
            if status == "missing-tarball":     return " mis"
