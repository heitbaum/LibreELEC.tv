...
| addons/addon-depends/rsyslog-depends/snappy | `0086-add-pkg-config-file.patch` | Sebastien Lavoie — [PR #86](https://github.com/google/snappy/pull/86) add a CMake pkg-config file, a redo of the abandoned [PR #55](https://github.com/google/snappy/pull/55); **both closed/rejected** — pwnall, 2019: "We will not add pkg-config support for now", upstream declining to pick a side in the C++ packaging split. Carried locally. Reworked to read `@PROJECT_VERSION@` rather than introduce a `SNAPPY_VER` variable, so it names no version and survives a bump untouched. Worth re-asking: the repo went dormant after that ruling and is merging community PRs again since 2026-09 |
