...
| addons/service/tvheadend43 | `0004-transcoding-init-the-filter-sink-after-setting-its-o.patch` | Rudi Heitbaum — `avfilter_graph_create_filter()` initializes the sink it creates and libavutil 60 (FFmpeg 8.0) made `av_opt_set*()` reject non-runtime options on an initialized object, so every buffersink format option fails and transcoding never starts (tvheadend issue 1764); allocate the sink with `avfilter_graph_alloc_filter()` and `avfilter_init_dict()` it once the options are applied; to submit to tvheadend/tvheadend |
| addons/service/tvheadend43 | `0005-transcoding-set-the-sink-formats-through-the-array-o.patch` | Rudi Heitbaum — libavfilter 12 (FFmpeg 9.0) removed buffersink's int-list/string format options, and from 10.6.100 the deprecated audio ones are ignored once the layout is set as a string, so set `pixel_formats`/`sample_formats`/`samplerates`/`channel_layouts` with `av_opt_set_array()`; to submit to tvheadend/tvheadend |
...
| addons/addon-depends/rsyslog-depends/snappy | `0086-add-pkg-config-file.patch` | Rudi Heitbaum, reworking Sebastien Lavoie — install a CMake pkg-config file so rsyslog's configure finds snappy. Lavoie's [PR #86](https://github.com/google/snappy/pull/86), itself a redo of the abandoned [PR #55](https://github.com/google/snappy/pull/55), was **closed/rejected** — pwnall, 2019: "We will not add pkg-config support for now", upstream declining to pick a side in the C++ packaging split. Re-raised 2026-09-19 as [PR #265](https://github.com/google/snappy/pull/265) — CLA and checks green, awaiting review. Reads `@PROJECT_VERSION@` instead of introducing Lavoie's `SNAPPY_VER`, so it names no version and survives a bump untouched |
