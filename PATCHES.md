...
| network/cifs-utils | `0001-fix-const-correctness-in-parse-options.patch` | Rudi Heitbaum — parse_options() const on data parameter was wrong; submitted to linux-cifs@vger.kernel.org. **Merged to upstream `next`** — drop when released past 7.6 |
...
| multimedia/media-driver | `1974-suppress-dangling-pointer-warning-in-mhw-state-heap-g.patch` | Rudi Heitbaum — [issue #1974](https://github.com/intel/media-driver/issues/1974) suppress GCC -Wdangling-pointer (not yet a PR) |
...
| devel/glibc | `0001-Linux--Only-define-OPEN-TREE-...` | Florian Weimer (Red Hat) — landed on the glibc 2.43 stable branch *after* the 2.43 release tarball; drop at glibc 2.44 (not on the 2.43 version match) |
