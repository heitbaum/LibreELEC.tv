...
* For simple tests, `[ expr1 -o expr2 ]`/`[ expr1 -a expr2 ]` is preferred over the extended form `[[ expr1 || expr2 ]]`/`[[ expr1 && expr2 ]]`. Use `[[ ]]` when short-circuit evaluation is required (not supported by `-a`/`-o`), or when regex (`=~`) or glob/pattern matching is needed — these require `[[ ]]`. See [test constructs](https://www.tldp.org/LDP/abs/html/testconstructs.html)
