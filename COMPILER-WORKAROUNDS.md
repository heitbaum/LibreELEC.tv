...
|---|---|---|---|---|
| aom | `packages/multimedia/aom/package.mk:23` | `TARGET_CFLAGS+=" -Wno-implicit-function-declaration"` (arm only) | gcc-14 erroring on neon declarations ([upstream bug](https://bugs.chromium.org/p/aomedia/issues/detail?id=3576)) — **last confirmed 3.13.3 / gcc-16**; tree now 3.14.1 | 2024-05 |
...
| fontconfig | `packages/x11/other/fontconfig/package.mk:29` | `sed -O3 → -O2` (CFLAGS and CXXFLAGS) | `-O3` causes correctness/stability issues in fontconfig — in upstream LibreELEC | ~2016 |
...
| heimdal | `packages/devel/heimdal/package.mk:19` | `CFLAGS+=" -Wno-error=implicit-function-declaration"` (host only) | without it, configure probe for `crypt` fails → linker error `undefined reference to 'crypt'` — **confirmed still needed / gcc-16** | ~2024 |
...
| mariadb-connector-c | `packages/databases/mariadb-connector-c/package.mk:26` | `CFLAGS+=" -Wno-discarded-qualifiers"` | glibc-2.43 made `strchr`/`strstr` C23-compliant (const-preserving return type), mariadb assigns result to non-const — **last confirmed 3.4.8 / gcc-16**; tree now 3.4.9 | 2026-01 |
| media-driver | `packages/multimedia/media-driver/package.mk:16` | `CXXFLAGS+=" -Wno-error=array-bounds="` | gcc-15 (≥20250330) build failure — **last confirmed 26.2.0 / gcc-16**; tree now 26.3.1 | 2025-04 |
...
| rsyslog | `packages/addons/service/rsyslog/package.mk:37` | `CFLAGS+=" -fcommon"` | gcc-10 changed default to `-fno-common`; multiple-definition link errors without this — in upstream LibreELEC | ~2020 |
...
| weston | `packages/wayland/weston/package.mk:45` | `sed removes -DNDEBUG from TARGET_CFLAGS` | weston requires assertions enabled at runtime — in upstream LibreELEC | ~2020 |
